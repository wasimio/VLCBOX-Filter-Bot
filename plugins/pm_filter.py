...
async def advantage_spoll_choker(bot, query):
    ...
    reply_msg_id = query.message.reply_to_message.id if (query.message and getattr(query.message, "reply_to_message", None)) else (query.message.id if query.message else None)
    movies = SPELL_CHECK.get(reply_msg_id) if reply_msg_id else None
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    if movie_ == "close_spellcheck":
        return await query.message.delete()
    if not movies or int(movie_) >= len(movies):
        return await query.answer("Spellcheck expired.", show_alert=True)
    movie = movies[(int(movie_))]
    movie = re.sub(r"[:\-]", " ", movie)
    movie = re.sub(r"\s+", " ", movie).strip()
    await query.answer(script.TOP_ALRT_MSG)
    chat_id = query.message.chat.id if (query.message and getattr(query.message, "chat", None)) else query.from_user.id
    gl = await global_filters(bot, query.message, text=movie)
    if gl == False:
        k = await manual_filters(bot, query.message, text=movie)
        if k == False:
            files, offset, total_results = await get_search_results(chat_id, movie, offset=0, filter=True)
            if files:
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    log_callback_debug(query)
    try:
        if query.message and getattr(query.message, "reply_to_message", None) and query.message.reply_to_message.from_user:
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 𝖲𝖾𝗅𝖾𝖼𝗍 𝖸𝖾𝖺𝗋 👇", callback_data="ident"
            )
        ],
    )
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if lang == "homepage":
        ...
        for yr in YEARS:
            if yr.lower() in search.lower():
                search = re.sub(rf'\b{re.escape(yr)}\b', '', search, flags=re.IGNORECASE).strip()
...
async def filter_episodes_cb_handler(client: Client, query: CallbackQuery):
    ...
    if lang == "homepage":
        ...
        for ep in EPISODES:
            if ep.lower() in search.lower():
                search = re.sub(rf'\b{re.escape(ep)}\b', '', search, flags=re.IGNORECASE).strip()
        search = f"{search} {lang}".strip()
        BUTTONS[key] = search
        files, offset, total_results = await get_search_results(chat_id, search, offset=0, filter=True)
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if lang == "homepage":
        ...
        for l_tag in LANGUAGES:
            if l_tag.lower() in search.lower():
                search = re.sub(rf'\b{re.escape(l_tag)}\b', '', search, flags=re.IGNORECASE).strip()
        search = f"{search} {lang}".strip()
        BUTTONS[key] = search
        files, offset, total_results = await get_search_results(chat_id, search, offset=0, filter=True)

    if not files:
        await query.answer("🚫 𝗡𝗼 𝗙𝗶𝗹𝗲 𝗪𝗲𝗿ᴇ 𝗙𝗼𝘂𝗻𝗱 🚫", show_alert=1)
        return
    temp.GETALL[key] = files
    settings = await get_settings(chat_id)
    pre = 'filep' if settings['file_secure'] else 'file'
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if seas == "homepage":
        ...
        # Strip existing season tags from search base if any
        season_patterns = [
            r"\bs\d+\b", r"\bseason\s*\d+\b", r"\bs\.\d+\b"
        ]
        base_search = search
        for sp in season_patterns:
            base_search = re.sub(sp, "", base_search, flags=re.IGNORECASE).strip()
        base_search = re.sub(r"\s+", " ", base_search).strip()

        # Extract season number from seas (e.g. "season 1" -> 1)
        match = re.search(r'\d+', seas)
        season_num = int(match.group()) if match else 1
        
        seas_s = f"s{season_num:02d}"
        seas_full = f"season {season_num}"
        ...
        for term in [seas_s, seas_full, seas_full0, seas_sn]:
            term_search = f"{base_search} {term}".strip()
            res, _, _ = await get_search_results(chat_id, term_search, max_results=15)
            for f in res:
                fid = str(f.get("file_id") or f.get("_id"))
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 𝖲𝖾𝗅𝖾𝖼𝗍 𝖰𝗎𝖺𝗅𝗂𝗍𝗒 👇", callback_data="ident"
            )
        ],
    )
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if qual == "homepage":
        ...
        for q_tag in QUALITIES:
            if q_tag.lower() in search.lower():
                search = re.sub(rf'\b{re.escape(q_tag)}\b', '', search, flags=re.IGNORECASE).strip()
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer(f"Jᴏɪɴ ᴏᴜʀ Bᴀᴄᴋ-ᴜᴘ {'ᴄʜᴀɴɴᴇʟs' if len(AUTH_CHANNEL) > 1 else 'ᴄʜᴀɴɴᴇʟ'} ᴍᴀʜɴ! 😒", show_alert=True)
            return
      ...
      #      return
        ...
        if query.from_user.id in ADMINS:
            try:
                ...
                logger.error(e)
                await query.answer("Error! User might have deleted account.", show_alert=True)
        else:
        ...
        if query.from_user.id in ADMINS:
            try:
                user = await client.get_users(int(from_user))
                content = query.message.text or "Request"
                try:
                    await query.message.edit_text(f"<b><strike>{content}</strike></b>", reply_markup=InlineKeyboardMarkup(btn))
                except Exception:
                    pass
                ...
                logger.error(f"Error in unavailable handler: {e}")
        ...
        if query.from_user.id in ADMINS:
            try:
                ...
                logger.error(f"Error in uploaded handler: {e}")
                await query.answer("Error processing request!", show_alert=True)
        else:
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        if query.from_user.id in ADMINS:
            try:
                ...
                logger.error(f"Error in already_available handler: {e}")
                await query.answer("Error processing request!", show_alert=True)
        else:
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('sᴜᴘᴘʀᴛ ɢʀᴏᴜᴘ', url=f'https://t.me/{SUPPORT_CHAT}')
        ],[
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://t.me/rickakhtar")
        ],[
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/rickakhtar")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        ...
        for VLCBox in movienamelist:
            try:
                ...
                pass
            if mv_rqst.startswith(VLCBox[0]):
                await auto_filter(client, VLCBox, msg, reply_msg, vj_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}"),
